...
$SS_{res}=SUM(y_i-y^{^}_i)^2$ is the sum of squared residuals, and $SS_{tot}=SUM(y_i-\bar{y})^2$ is the total sum of squares.


The solution to the problem of overfitting is to use Adjusted R Squared instead of regular R Squared when comparing models with different numbers of predictors. This way, we can ensure that we are selecting a model that provides a good fit to the data without including unnecessary variables.

R square help tell us how good of the fit is of the line compraed to the average line. R squared can be negative. In adjusted R2 formula, p represents the number of regressors.
\end{document}