...
Same trick as ../frontend/build.py, pointed at the 3D viewer instead. Both
frontends read the same exported policy.json, so retraining updates them
together:

    python ../football_rl.py --episodes 12000 --seed 42 --export policy.json
    python ../frontend/build.py policy.json
